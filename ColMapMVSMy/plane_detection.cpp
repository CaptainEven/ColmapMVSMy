...
bool PlaneDetection::readDepthMat(const cv::Mat& depth_mat)
{
	if (depth_mat.empty() || depth_mat.depth() != CV_32FC1)
	{
		printf("Warning: cannot read depth image.\
				No such a file, or the image format is not 16UC1\n");
		return false;
	}

	const int rows = depth_mat.rows, cols = depth_mat.cols;
	int vertex_idx = 0;
	for (int i = 0; i < rows; ++i)
	{
		for (int j = 0; j < cols; ++j)
		{
			double z = (double)(depth_mat.at<float>(i, j));
			if (_isnan(z))
			{
				cloud.vertices[vertex_idx++] = VertexType(0, 0, z);
				continue;
...
bool PlaneDetection::readCloud(const cv::Mat_<cv::Vec3f>& my_cloud)
{
	if (my_cloud.empty())
	{
		printf("Warning: empty cloud\n");
		return false;
	}

	int vertex_idx = 0;
	for (int r = 0; r < my_cloud.rows; ++r)
	{
		// 一行点云的指针
		const cv::Vec3f* pt_ptr = my_cloud.ptr<cv::Vec3f>(r);

		for (int c = 0; c < my_cloud.cols; ++c)
		{
			const float& x = pt_ptr[c][0];
			const float& y = pt_ptr[c][1];
			const float& z = pt_ptr[c][2];
			if (_isnan(z) || z <= 0.0f)
			{
				cloud.vertices[vertex_idx++] = VertexType(0, 0, z);
				continue;
...
void PlaneDetection::prepareForMRF()
{
	...
	cv::Mat& mat_label = plane_filter.membershipImg;  // labels
	for (int row = 0; row < kDepthHeight; ++row)
	{
		for (int col = 0; col < kDepthWidth; ++col)
		{
				...
				|| (col + 1 < kDepthWidth && mat_label.at<int>(row, col + 1) != label))
			{
				...
				for (int y = max(row - kNeighborRange, 0); y < min(kDepthHeight, row + kNeighborRange); ++y)
				{
					for (int x = max(col - kNeighborRange, 0); x < min(kDepthWidth, col + kNeighborRange); ++x)
					{
						// If a pixel is not on any plane, then it is not a boundary pixel.
						if (mat_label.at<int>(y, x) == plane_num_)
						{
							continue;
						}

						pixel_boundary_flags_[y * kDepthWidth + x] = true;
					}
				}
			}
		}
