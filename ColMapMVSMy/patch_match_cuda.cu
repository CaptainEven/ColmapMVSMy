...
namespace colmap {
	namespace mvs {
		...
		//cudaReadModeNormalizedFloat进行类型转化，用tex2D取到的值是归一化的【0，1】
...
#pragma unroll//可以将local_iamge想象成二维的，每一行大小为3*ThreadPB，有kWindowSize列
					//每个线程读一行中的三个值，读取窗口大小的高度
					for (int j = 0; j < 3; ++j) {
...
#pragma unroll
		...
		template <int kWindowSize, bool kGeomConsistencyTerm = false,
			...
			for (int row = 0; row < cost_map.GetHeight(); ++row) {
				...
				////阈值判断
				//int numLessCosts = 0;//Patch块cost值小于某个阈值的个数
				//for (int image_id = 0; image_id < cost_map.GetDepth(); ++image_id)
				//{
				//	if (cost_map.Get(row, col, image_id) <= 0.4f){
				//		++numLessCosts;
				//	}
				//}
				//if (numLessCosts >= 2)
				...
				//	prev_param_state.depth = depth_map.Get(row, col);
				//	float curNormals[3]; normal_map.GetSlice(row, col, curNormals);
				//	for (int i = 0; i < 3; ++i) {
				//		prev_param_state.normal[i] = curNormals[i];
				//	}
				//	continue;//直接跳过，处理下一行
				//}
				...
				////少了两组(depth,normal)分别是(cur,perturb),(perturb,cur)
...
#undef CALL_RUN_FUNC
			...
			for (size_t r = 0; r < mask.GetHeight(); ++r) {
				for (size_t c = 0; c < mask.GetWidth(); ++c) {
					...
					for (size_t d = 0; d < mask.GetDepth(); ++d) {
						if (mask.Get(r, c, d)) {
							pixel_consistent_image_ids.push_back(problem_.src_img_ids[d]);
						}
					}
		...
		void PatchMatchCuda::RunWithWindowSize()
		{
			...
			const int num_iterations = options_.geom_consistency ? options_.num_geometric_iterations : options_.num_photometric_iteratoins;

			const float total_num_steps = num_iterations * 4;
			...
			// 输出初始化的深度和法向map图
			if (bOutPutMessage && problem_.ref_img_id == 0)
				//if (bOutPutMessage)
			{
				if (!options_.geom_consistency)
				{
					cv::imwrite("df_0_0.jpg", GetDepthMap().ToBitmap(2, 98));
					cv::imwrite("nf_0_0.jpg", GetNormalMap().ToBitmap());
				}
				else
				{
					cv::imwrite("dg_0_0.jpg", GetDepthMap().ToBitmap(2, 98));
					cv::imwrite("ng_0_0.jpg", GetNormalMap().ToBitmap());
				}
			}
			...
			for (int iter = 0; iter < num_iterations; ++iter)
			{
				...
				for (int sweep = 0; sweep < 4; ++sweep)
				{
					...
					const bool last_sweep = iter == num_iterations - 1 && sweep == 3;
...
#undef CALL_SWEEP_FUNC
					...
					// 把每一次迭代后的深度和法向map图输出到本地查看
					if (bOutPutMessage  && problem_.ref_img_id == 0)
						//if (bOutPutMessage)
					{
						cv::Mat depthmapImage, normalmapImage;
						depthmapImage = GetDepthMap().ToBitmap(2, 98);
						normalmapImage = GetNormalMap().ToBitmap();
						if (sweep != 3)
						{
							for (int i = 0; i < sweep + 1; i++)
							{
								cv::transpose(depthmapImage, depthmapImage);
								cv::flip(depthmapImage, depthmapImage, 1);//顺时针旋转矩阵

								cv::transpose(normalmapImage, normalmapImage);
								cv::flip(normalmapImage, normalmapImage, 1);
							}
						}
						char path_1[20];
						char path_2[20];
						if (!options_.geom_consistency)
						{
							sprintf_s(path_1, "df_%d_%d.jpg", iter + 1, sweep + 1);
							sprintf_s(path_2, "nf_%d_%d.jpg", iter + 1, sweep + 1);
						}
						else
						{
							sprintf_s(path_1, "dg_%d_%d.jpg", iter + 1, sweep + 1);
							sprintf_s(path_2, "ng_%d_%d.jpg", iter + 1, sweep + 1);
						}

						cv::imwrite(path_1, depthmapImage);
						cv::imwrite(path_2, normalmapImage);
					}
				}
		...
		void PatchMatchCuda::InitRefImage() {
			...
			cv::Mat ref_image_mat = ref_image.GetBitmap();
			std::vector<uint8_t> ref_image_array(ref_height_*ref_width_);
			ref_image_array.assign(ref_image_mat.datastart, ref_image_mat.dataend);
		   ...
		   //const cudaChannelFormatDesc channelDesc = cudaCreateChannelDesc<uint8_t>();
			...
			checkCudaErrors(cudaBindTextureToArray(ref_image_texture, ref_image_device_->GetPtr()));
		}
		...
		void PatchMatchCuda::InitSourceImages() {
			...
			size_t max_width = 0;
			size_t max_height = 0;
			for (const auto image_id : problem_.src_img_ids) {
			...
			// Upload source images to device.
			{
				...
				std::vector<uint8_t> src_images_host_data(
					static_cast<size_t>(max_width * max_height *
						problem_.src_img_ids.size()),
					kDefaultValue);
				for (size_t i = 0; i < problem_.src_img_ids.size(); ++i) {
					const Image& image = problem_.images->at(problem_.src_img_ids[i]);
					...
					for (size_t r = 0; r < image.GetHeight(); ++r) {
						memcpy(dest, bitmap.ptr<uchar>(r), image.GetWidth() * sizeof(uint8_t));
						dest += max_width;
					}
				}
				...
				src_images_device_.reset(new CudaArrayWrapper<uint8_t>(
					max_width, max_height, problem_.src_img_ids.size()));
				...
				checkCudaErrors(cudaBindTextureToArray(src_images_texture,
			...
			if (options_.geom_consistency) {
				...
				for (size_t i = 0; i < problem_.src_img_ids.size(); ++i) {
					const DepthMap& depth_map =
						problem_.depth_maps->at(problem_.src_img_ids[i]);
					float* dest =
				...
				//const cudaChannelFormatDesc channelDesc1 = cudaCreateChannelDesc<float>();
				...
				checkCudaErrors(cudaBindTextureToArray(src_depth_maps_texture,
		...
		void PatchMatchCuda::InitTransforms() {
			...
			checkCudaErrors(cudaMemcpyToSymbol(ref_K, ref_K_host_[0], sizeof(float) * 4, 0,
				cudaMemcpyHostToDevice));
			checkCudaErrors(cudaMemcpyToSymbol(ref_inv_K, ref_inv_K_host_[0],
				sizeof(float) * 4, 0,
				cudaMemcpyHostToDevice));
			...
			for (size_t i = 0; i < 4; ++i) {
				...
				poses_device_[i].reset(new CudaArrayWrapper<float>(
					kNumTformParams, problem_.src_img_ids.size(), 1));
		...
		__global__ void InitSparse(GpuMat<float> depth_map, GpuMat<float> normal_map, GpuMat<float> prev_sel_prob_map,
			float *sparsePoints, float *sparseNormals, int *tracks,
			const size_t num, const size_t trackNum)
		{
			for (int i = 0, j = 0; i < num; i = i + 3, j++)
			{
				const float sp[3] = { sparsePoints[i], sparsePoints[i + 1], sparsePoints[i + 2] };
				const float sn[3] = { sparseNormals[i], sparseNormals[i + 1], sparseNormals[i + 2] };
				depth_map.Set(sp[0], sp[1], sp[2]);
				normal_map.SetSlice(sp[0], sp[1], sn);

				for (int k = j + 1; tracks[k] != -1; k++)//-1。。。-1。。。-1数据记录在每两个-1之间
				{
					prev_sel_prob_map.Set(sp[0], sp[1], tracks[k], 1.0f);
				}
			}
		}
		...
		void PatchMatchCuda::InitWorkspaceMemory() {
			...
			if (options_.geom_consistency) {
				const DepthMap& init_depth_map =
					problem_.depth_maps->at(problem_.ref_img_id);
			...
			global_workspace_.reset(
				new GpuMat<float>(ref_max_dim, problem_.src_img_ids.size(), 2));
			...
			if (!options_.geom_consistency && options_.bUse_sparse_points)
			{
				const size_t size = problem_.sparsePoints.size();
				const size_t trackNum = problem_.tracks.size();
				float *sparsePoints, *sparseNormals;
				int *tracks;
				checkCudaErrors(cudaMalloc((void **)&sparsePoints, size * sizeof(float)));
				checkCudaErrors(cudaMemcpy(sparsePoints, &problem_.sparsePoints[0], size * sizeof(float), cudaMemcpyHostToDevice));
				checkCudaErrors(cudaMalloc((void **)&sparseNormals, size * sizeof(float)));
				checkCudaErrors(cudaMemcpy(sparseNormals, &problem_.sparseNormals[0], size * sizeof(float), cudaMemcpyHostToDevice));
				checkCudaErrors(cudaMalloc((void **)&tracks, trackNum * sizeof(int)));
				checkCudaErrors(cudaMemcpy(tracks, &problem_.tracks[0], trackNum * sizeof(int), cudaMemcpyHostToDevice));


				dim3 block(1, 1, 1);
				InitSparse << <1, block >> > (*depth_map_, *normal_map_, *prev_sel_prob_map_,
					sparsePoints, sparseNormals, tracks, size, trackNum);

				//CUDA_CHECK_ERROR();

				checkCudaErrors(cudaFree(sparsePoints));
				checkCudaErrors(cudaFree(sparseNormals));
				checkCudaErrors(cudaFree(tracks));
			}
		...
		void PatchMatchCuda::Rotate() {
			...
			//const cudaChannelFormatDesc channelDesc = cudaCreateChannelDesc<uint8_t>();
			checkCudaErrors(cudaUnbindTexture(ref_image_texture));
			checkCudaErrors(
			...
			prev_sel_prob_map_.reset(
				new GpuMat<float>(width, height, problem_.src_img_ids.size()));
			...
			const cudaChannelFormatDesc channelDesc1 = cudaCreateChannelDesc<float>();
			checkCudaErrors(cudaUnbindTexture(poses_texture));
			checkCudaErrors(cudaBindTextureToArray(
				poses_texture, poses_device_[rotation_in_half_pi_]->GetPtr()));

			// Rotate calibration.
			checkCudaErrors(cudaMemcpyToSymbol(ref_K, ref_K_host_[rotation_in_half_pi_],
				sizeof(float) * 4, 0,
				cudaMemcpyHostToDevice));
			checkCudaErrors(
