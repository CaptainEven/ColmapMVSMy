...
cv::Mat GuidedFilterImpl::filter(const cv::Mat& p, int depth)
{
	...
	else
	{
		...
		for (std::size_t i = 0; i < pc.size(); ++i)
			pc[i] = this->filterSingleChannel(pc[i]);
...
GuidedFilterMono::GuidedFilterMono(const cv::Mat& origI,
...
GuidedFilter::GuidedFilter(const cv::Mat& I, int r, double eps)
{
	...
	uint win_size = 2 * r + 1;
	//std::cout << "Win size: " << win_size << "¡Á" << win_size << std::endl;
